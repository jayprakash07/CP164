...
class Sorted_List:
    ...
    def clean(self):
        ...
        while check is not None:
            current = check._next
            while current is not None and current._value == check._value:
                check._next = current._next
                self._count -= 1
                current = current._next
            if current is None:
                self._rear = check
    ...
    def intersection(self, source1, source2):
        ...
        -------------------------------------------------------
        """
        node1, node2 = source1._front, source2._front
        ...
        while node1 is not None and node2 is not None:
            if node1._value < node2._value:
                ...
                if self._front is None:
                    self._front = _SL_Node(node1._value, None)
                    self._rear = self._front
                    self._count += 1
                elif self._rear._value < node1._value:
                    self._rear._next = _SL_Node(node1._value, None)
                    self._rear = self._rear._next
                    self._count += 1
                else:
                    node1, node2 = node1._next, node2._next
        return
    ...
    def __eq__(self, target):
        ...
        if self._count == target._count:
            check, check2 = self._front, target._front
            while check is not None:
