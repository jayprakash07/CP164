...
def read_genres():
    ...
    user_number = input('Enter a genre number (ENTER to quit): ')
    if user_number == "":
        print('Error: must have at least one genre')

    while user_number != '' or len(genres) == 0:
        if not user_number.isnumeric():
            print('Error: not a positive number')
        elif int(user_number) > 10:
            print('Error: input must be <= 10')
        elif int(user_number) in genres:
            print('Error: genre already chosen')
        else:
            genres.append(int(user_number))
        user_number = input('Enter a genre number (ENTER to quit): ')
        genres.sort()
